'use strict';
const bannerPrice = require('../models/banner_ads_displays');
...
let FindBannerPrice = () => {
    return new Promise((resolve, reject) => {
        bannerPrice.find({}, function (err, price) {
...
let FindOnePrice = (id) => {
    return new Promise((resolve, reject) => {
        bannerPrice.findOne({_id: id}, function (err, price) {
            if (err) return reject(err);
            resolve(price);
        });
    });
};
exports.get_banner_price = function (req, res) {
    FindBannerPrice()
        .then(
            price => {
                if (price) {
...
let Err = [];
Err.push("1. update wallet success, create data buy banner fail");
Err.push("2. update wallet fail");
Err.push("3. Not enough money");
Err.push("4. Upload Image banner fail");
exports.insert_banner_ads = function (req, res) {
